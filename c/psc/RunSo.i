...
int main(int argc, const char *argv[])
{
 ...
 case 'i': {
  int (*func_int)() = func;
  i = 2; while (++i < argc - 1) { switch (argv[i][0]) { case 'i': asm volatile("push %0" :: "r"(atol(&argv[i][1]))); rsp += 8; break; case 'd': atof(&argv[i][1]); asm volatile("sub $8, %rsp\n" "fstpl (%rsp)"); rsp += 8; break; case 's': asm volatile("push %0" :: "r"(&argv[i][1])); rsp += 8; break; default : printf("error argument type\n"); goto exit_runso; } };
  int ret = func_int();
