...
// 构造函数的特点有两个
// 函数体内部使用了 this 关键字，代表了所要生成的对象的实例
...
//     at Object.<anonymous> (/root/Code/js/oop/new.js:73:15)
...
// 上面代码中，调用 Vehicle 构造函数时，忘了加上 new 命令。结果，变量 v 变成了 undefined,
// 而 price 属性变成了全局变量。因此，应该非常小心，避免不使用 new 命令、直接调用构造函数

// 为了保证构造函数必须与 new 命令一起使用，一个解决办法是，构造函数内部使用严格模式，即
// 第一行加上 use strict。这样的话，一旦忘了使用 new 命令，直接调用构造函数就会报错
...
// 上面代码的 Fubar 为构造函数，use strict 命令保证了该函数在严格模式下运行。由于严格模式中，函数内部的
// this 不能指向全局对象，默认等于 undefined, 导致不加 new 调用会报错 (JavaScript 不允许对 undefined 添加属性)
...
// 如果构造函数内部有 return 语句，而且 return 后面跟一个对象，new 命令会返回 return 语句指定的对象；否则，
// 就会不管 return 语句，返回 this 对象
...
// 上面代码中，构造函数 Vehicle 的 return 语句返回一个数值。这时，new 命令就会忽略这个 return 语句，
// 返回 "构造" 后的 this 对象
...
// 这是因为 new 命令总是返回一个对象，要么是实例对象，要么是 return 语句指定的对象。本例中，
// return 语句返回的是字符串，所以 new 命令就忽略了该语句
...
// new.target
// 函数内部可以使用 new.target 属性。如果当前函数是 new 命令调用，new.target 指向当前函数，
// 否则为 undefined
...
// f();
// Uncaught Error: 请使用 new 命令调用！

// 上面代码中，构造函数 f 调用时，没有使用 new 命令，就抛出一个错误


// Object.create() 创建实例对象
// 构造函数作为模板，可以生成实例对象。但是有时拿不到构造函数，只能拿到一个现有的对象。
