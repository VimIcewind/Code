...
(define (read-line)
  (define (read-char-until-newline)
    (let ((ch (read-char)))
      (if (eof-object? ch)    ; 如果遇到文件结束符
        (void)              ; 返回空值
        (if (= (char->integer ch) 10)  ; 如果遇到换行符 (newline)
          '()              ; 返回空列表，结束读取
          (cons ch (read-char-until-newline))))))  ; 继续读取下一个字符
  (let ((chars (read-char-until-newline)))
    (list->string chars)))   ; 将字符列表转换为字符串
...
(define (keep-matching-items lst pred)
  (filter pred lst))

;;; Practical main function. tail recursive.
(define (mastermind-rec an0 candidates)
  (if (null? candidates)
    (display "Error. You gave wrong score for my guess, probably.")
